...
> 모든 화면이 **Light / Dark 테마 양쪽** 으로 작동합니다. 사이드바 상단 "테마 전환" 버튼 한 번 클릭으로 즉시 변경되며, 선택은 영구 저장됩니다.

### 1. 기능 설명 페이지

처음 진입 시 (모델·이미지 로드 전) 또는 검수 도중 사이드바 **"기능 설명 페이지 보기"** 토글로 언제든 볼 수 있습니다. 시작하기 / 검수 작업 / 자동 복구 / 단축키 등 전체 흐름이 한 페이지에 정리되어, 처음 사용자도 이 페이지만 보면 전체 흐름 이해 가능.

![기능 설명 페이지](docs/screenshots/01_guide.png)

### 2. 사이드바

세션 / 모델·이미지 업로드 / 추론 실행 / 보기 모드 / 검수 옵션 / 마지막 설정 자동 로드 + 테마 전환 토글까지 모든 컨트롤이 한 곳에 정리.

![사이드바](docs/screenshots/02_sidebar.png)

### 3. 분류 검수 (Classifier)

이미지마다 분류 모델의 top-1 예측 + 신뢰도 표시. 운영자가 ○ 정확 / ✕ 오류 / △ 애매 판정 + (오류인 경우) 정답 클래스 직접 선택.

![분류 검수](docs/screenshots/03_inspect_classifier.jpg)

### 4. 폴리곤 검수 (Segmenter)

세그멘테이션 모델이 예측한 폴리곤을 캔버스에 표시. 정점 이동 / 추가 / 삭제, 새 영역 그리기, 클래스 변경 모두 직관적인 마우스 조작으로 가능 (자체 빌드 React + Konva.js 컴포넌트).

![폴리곤 검수](docs/screenshots/04_inspect_segmenter.jpg)

### 5. 자동 복구 — 인프라엑스 자비스 복구 페이지

새로고침 시 마지막 모델 + 이미지 + 검수 결과를 자동 복원. 모델·이미지·검수 결과·마지막 위치까지 일괄 복원. 멈춘 것 같으면 "복구 중단하고 초기 화면으로" 버튼으로 탈출 가능.

![자동 복구](docs/screenshots/05_auto_restore.png)

### 6. 세션 통계 대시보드

검수 진행률, verdict (정확 / 오류 / 애매) 분포, 분류기 신뢰도 분포, 모델 → 사람 정정 매트릭스, 시간대별 검수 처리량 등.

![세션 통계](docs/screenshots/06_stats.png)

### 7. 결과 내보내기 (Export)

YOLO Segmentation / YOLO Classification / COCO 3종 포맷 동시 생성. 검수 완료한 것만 필터 + 정정된 것만 필터 가능. 단일 zip 다운로드 + 서버측 저장 폴더 옵션.

![내보내기](docs/screenshots/07_export.png)
